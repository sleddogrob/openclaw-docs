---
layout: home
hero:
  name: "My OpenClaw Notes"
  tagline: "Things I wish someone told me before!"
  image:
    src: /380x207-claw.png
    alt: OpenClaw Logo
  actions:
    - theme: brand
      text: Get Started
      link: /guide/
    - theme: al
      text: View on GitHub
      link: https://github.com/sleddogrob/openclaw-docs
features:
  - icon: ⚡
    title: Feature One
    details: Describe the first key feature or benefit of OpenClaw here.
  - icon: 🔧
    title: Feature Two
    details: Describe the second key feature or benefit here.
  - icon: 🌍
    title: Feature Three
    details: Describe the third key feature or benefit here.
---

Looking for the official OpenClaw [site](https://openclaw.ai/)?
